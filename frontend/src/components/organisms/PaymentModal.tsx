...
const PaymentModal = ({ isOpen, onClose, onSubscribe }: PaymentModalProps) => {
	...
	const plans = [
		...
		},
		{
			...
			price: "24,900",
			period: "원",
			billingText: "연간 청구 (월 2,075원)",
			buttonText: "1년 구독 시작하기",
			features: ["자동 생성 무제한", "병동 당 간호사 무제한", "광고 제거"],
			popular: false,
		},
	];
	...
	return (
		<>
			...
			>
				<div
					className="relative bg-base-white rounded-[1.25rem] shadow-lg p-8 w-[90%] max-w-[60rem] min-h-[33rem]"
				...
				>
					<div className="flex justify-between items-center mb-6">
						<button
							...
							className="absolute top-6 right-6 text-base-foreground hover:text-primary-dark transition-colors"
					...
					<div className="flex flex-col items-center justify-center text-center mb-10">
						<div className="inline-flex items-center gap-2 bg-primary-10 text-primary-dark px-4 py-1 rounded-full mb-3">
							<FaRegStar className="text-primary" />
							<span className="font-medium">얼리버드 특별 혜택</span>
						</div>
						<div className="text-2xl font-bold text-base-foreground mb-2">
							지금 구독하면 자동 생성 기능{" "}
							<span className="text-primary-dark">1개월 무료</span>
						</div>
						<div className="text-center text-base-foreground/80 max-w-2xl">
							초기 구독자에게만 제공되는 특별 혜택으로, 병동 단위 결제로 간호사
							전원이 이용할 수 있습니다.
					...
					<div className="mt-8 text-center text-sm text-base-foreground/60">
