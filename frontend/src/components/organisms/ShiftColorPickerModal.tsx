...
// 파스텔톤 위주의 확장된 색상 옵션
const colorOptions: Record<DutyType, { className: string; hex: string; text?: string }[]> = {
  day: [
...
function getTextColorClass(bgClass: string) {
  if (bgClass.includes('gray') || bgClass.includes('base-white') || bgClass.includes('lime') || bgClass.includes('yellow') || bgClass.includes('sky') || bgClass.includes('lightBlue')) {
    return 'text-gray-800';
  }
  if (bgClass.includes('green')) {
    return 'text-green-800';
  }
  if (bgClass.includes('red') || bgClass.includes('rose') || bgClass.includes('pink') || bgClass.includes('orange')) {
    return 'text-red-800';
  }
  if (bgClass.includes('blue') || bgClass.includes('indigo') || bgClass.includes('cyan')) {
    return 'text-blue-800';
  }
  if (bgClass.includes('purple') || bgClass.includes('violet') || bgClass.includes('fuchsia')) {
...
// DutyBadgeKor 수정
const DutyBadgeKorLike = ({ type, colorClass, textClass }: { type: DutyType; colorClass: string; textClass?: string }) => {
...
const ShiftColorPickerModal = ({
  ...
  const [localColors, setLocalColors] = useState<Record<DutyType, string>>(dutyColors);
      ...
      >
        ...
        <div className="px-6 pt-4 pb-2 bg-gray-50 border-b">
          ...
          <div className="flex flex-wrap gap-2">
            {dutyTypes.map(type => (
              <DutyBadgeKorLike key={type} type={type} colorClass={localColors[type]} />
            ))}
          </div>
        </div>
        ...
        <div className="px-6 py-4 max-h-[400px] overflow-y-auto">
          <div className="space-y-6">
            {dutyTypes.map(type => (
              <div key={type} className="pb-4 border-b border-gray-100 last:border-b-0">
                <div className="flex items-center gap-3 mb-3">
                  <DutyBadgeKorLike type={type} colorClass={localColors[type]} />
                </div>
