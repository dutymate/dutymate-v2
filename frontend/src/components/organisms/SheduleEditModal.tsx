...
const ScheduleEditModal = ({ 
  ...
  const [startTime, setStartTime] = useState(initialData?.startTime || '오전 09:00');
  ...
  const colorOptions = [
    ...
    { name: '퍼플', value: 'purple', bg: 'bg-purple-500', dot: 'bg-purple-500' },
    ...
    { name: '옐로우', value: 'yellow', bg: 'bg-yellow-400', dot: 'bg-yellow-400' },
    { name: '토마토', value: 'tomato', bg: 'bg-orange-500', dot: 'bg-orange-500' },
    { name: '인디고', value: 'indigo', bg: 'bg-indigo-500', dot: 'bg-indigo-500' }
  ];
  ...
  const TimePicker = ({
    ...
    const [activeDropdown, setActiveDropdown] = useState<'period' | 'hour' | 'minute' | null>(null);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        {/* 헤더 */}
        <div className={"bg-primary-30 px-6 py-2 text-white flex justify-between items-center"}>
        ...
        <div className="p-6">
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 relative">
              <div>
                ...
                <div 
                  ...
                  onClick={isEditable ? () => setActiveTimePicker('start') : undefined}
                >
              ...
              <div>
                ...
                <div 
                  className={`w-full px-4 py-3 rounded-lg border flex items-center ${isEditable ? 'border-gray-300 cursor-pointer' : 'bg-gray-50 border-gray-200'}`}
                  onClick={isEditable ? () => setActiveTimePicker('end') : undefined}
                >
                  <span className="mr-2">⏰</span>
                  <span>{endTime}</span>
                </div>
            ...
            <div>
              ...
              {isEditable ? (
                ...
                <div className="flex items-center">
                  <span className={`w-5 h-5 rounded-full ${colorOptions.find(c => c.value === color)?.dot || 'bg-blue-500'} mr-2`}></span>
                  <span>{colorOptions.find(c => c.value === color)?.name || '기본'}</span>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className={`flex items-center w-full px-4 py-3 rounded-lg border ${isEditable ? 'border-gray-300' : 'bg-gray-50 border-gray-200'}`}>
                <span className="mr-2">📍</span>
                <input
                  type="text"
